 ...
 *
 * RG: LITMUS MEMGUARD VERSION (litmus_memguard.c)
 * We don't wake up the throttle thread to run within added memguard_scheduler
 * Instead we just set a flag and ask the core to rescehdule
 * Within litmus, if that flag is set, we then keep choosing to execute the throttle thread ourselves until
 * the flag is unset in the replenishment
 */
...
static int throttle_thread(void *arg)
{
	...
	while (!kthread_should_stop() && cpu_online(cpunr)) {
		...
		/* RG: we make this thread always awake, and we just schedule it ourselves instead */
...
int init_module( void )
{
	...
	for_each_online_cpu(i) {
		...
		/* RG: initialize our litmus flags to 0 */
		litmus_throttled[i] = 0;
	...
	litmus_throttled_ptr = &litmus_throttled;
