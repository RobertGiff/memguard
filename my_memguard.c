...
//#include <litmus/sched_psn_edf.h>
...
unsigned long litmus_lock_flags;
...
static void memguard_on_each_cpu_mask(const struct cpumask *mask,
	...
	/* hmm, this function requires that interrupts remain ENABLED, not disabled, preempte must be disabled however */
...
static inline int convert_events_to_mb(u64 events) {
	...
	int mb =
		div64_u64(events * CACHE_LINE_SIZE * 1000000 + (divisor - 1), divisor);
	return mb;
}
...
static inline void print_current_context(void) {
	trace_printk("in_interrupt(%ld)(hard(%ld),softirq(%d)"
			...
			in_interrupt(), in_irq(), (int)in_softirq(), (int)in_nmi(),
			(int)irqs_disabled());
}
...
static int mg_nr_bwlocked_cores(void) {
	...
	for_each_cpu(i, global->throttle_mask) {
		struct task_struct *t =
			(struct task_struct *)per_cpu_ptr(core_info, i)->throttled_task;
		if (t && t->bwlock_val > 0)
			nr += t->bwlock_val;
	}
	return nr;
...
void update_statistics(struct core_info *cinfo) {
	...
	if (cinfo->exclusive_mode) {
		...
		/* used time */
		exclusive_ns = (TM_NS(ktime_get()) - TM_NS(cinfo->exclusive_time));
	...
	DEBUG_PROFILE(trace_printk("%lld %d %p CPU%d org: %d cur: %d period: %ld, throttled_time_ns: %lld, num_throttled: %d\n",
				new, used, cinfo->throttled_task,
				smp_processor_id(), cinfo->budget,
				cinfo->cur_budget, cinfo->period_cnt,
				cinfo->overall.throttled_time_ns,
				cinfo->overall.throttled));
}
...
static void event_overflow_callback(struct perf_event *event,
	...
	//trace_printk("CPU%d, MEMGUARD, LOCKING CORE\n", smp_processor_id());
	//atomic_set(local_litmus_throttle_ptr, 1);
...
static void __newperiod(void *info) {
	...
	if (period == global->period_cnt) {
		...
		hrtimer_start_range_ns(&global->hr_timer, new_expire, 0,
...
static void period_timer_callback_slave(void *info) {
	...
	DEBUG(trace_printk("%p|New period %ld. budget=%d\n",
				cinfo->throttled_task, cinfo->period_cnt, cinfo->cur_budget));
	...
	/* unlock litmus scheduler */
	//trace_printk("CPU[%d], MEMGUARD, UNLOCKING CORE\n", smp_processor_id());
	//atomic_set(local_litmus_throttle_ptr, 0);
}
...
enum hrtimer_restart period_timer_callback_master(struct hrtimer *timer) {
	...
	if (orun > 1)
		DEBUG(trace_printk("ERR: timer overrun %d at period %ld\n",
					orun, new_period));
#if USE_BWLOCK
	global->bwlocked_cores = mg_nr_bwlocked_cores();
#endif
	memguard_on_each_cpu_mask(global->active_mask, period_timer_callback_slave,
			(void *)new_period, 0);
...
static void disable_counters(void) { on_each_cpu(__disable_counter, NULL, 0); }
...
static void start_counters(void) { on_each_cpu(__start_counter, NULL, 0); }
...
void __update_budget(void *info) {
	struct core_info *cinfo = this_cpu_ptr(core_info);
...
static ssize_t memguard_limit_write(struct file *filp, const char __user *ubuf,
...
void memguard_set_limit(int use_mb, ...) {
	va_list bw;
	int i;
	unsigned long events;
	...
	for_each_online_cpu(i) {
		...
		if (use_mb)
			events = (unsigned long)convert_mb_to_events(n);
		else
			events = n;
...
static int memguard_init_debugfs(void) {
	...
	debugfs_create_file("usage", 0666, memguard_dir, NULL, &memguard_usage_fops);
	return 0;
}
...
static int throttle_thread(void *arg) {
	...
	/* set the cpu to the current cpu */
	//current->rt_param.task_params.cpu = smp_processor_id();

	//sched_setscheduler(current, SCHED_FIFO, &param);
	sched_setscheduler(current, SCHED_MEMGUARD, &param);
	//if (sched_setscheduler(current, SCHED_LITMUS, &param) != 0)
	//	trace_printk("Error setting scheduler to SCHED_LITMUS\n");
	//else
	//	trace_printk("Setting thread to litmus scheduler...SUCCESS\n");

	while (!kthread_should_stop() && cpu_online(cpunr)) {

		DEBUG(trace_printk("wait an event\n"));
		wait_event_interruptible(cinfo->throttle_evt,
				cinfo->throttled_task || kthread_should_stop());

		DEBUG(trace_printk("CPU%d got an event, spinning or exiting\n",
					smp_processor_id()));
...
static int __init init_memguard(void) {
	int i;
	unsigned long flags;

	struct memguard_info *global = &memguard_info;

	printk("Initializing Memguard!\n");
	...
	for_each_online_cpu(i) {
		...
		if (g_budget_mb[i] == 0)
			g_budget_mb[i] = 100000; /* original membw */
			//g_budget_mb[i] = 1000; /* original membw */
		...
		cinfo->throttle_thread =
			kthread_create_on_node(throttle_thread, (void *)((unsigned long)i),
					cpu_to_node(i), "kthrottle_memguard/%d", i);
...
static void cleanup_memguard(void) {
	int i;
	...
	/* if we throttled litmus, unthrottle */
	//if (atomic_read(local_litmus_throttle_ptr) > 0) {
	//	trace_printk("CPU%d, MEMGUARD, UNLOCKING CORE IN CLEAN UP\n",
	//			smp_processor_id());
	//	atomic_set(local_litmus_throttle_ptr, 0);
	//}
...
module_init(init_memguard);
module_exit(cleanup_memguard);
